...
function run(
    ...
    if (!$context->quiet) {
        $output = $errorOutput = GlobalHelper::getOutput();
        if ($output instanceof ConsoleOutput) {
            $errorOutput = $output->getErrorOutput();
        }
        $callback = static function ($type, $bytes) use ($output, $errorOutput, $callback, $process) {
            if (Process::OUT === $type) {
                $output->write($bytes);
            } else {
        ...
        $callback = fn ($type, $bytes) => $callback($type, $bytes, $process);
    }
